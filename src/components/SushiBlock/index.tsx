...
import { addItem } from '../../store/slices/cart/cartSlise';
...
const SushiBlock = ({ id, name, price, imageUrl, sizes, types }: SushiBlockProps) => {

    const typeNames = ['обычное', 'спайси'];
    ...
    return (
        <div className="sushi-block">
            ...
            <div className="sushi-block__selector">
                ...
                <ul>
                    {sizes.map((size, i) => (
                        <li key={size} onClick={() => setActiveSize(i)} className={activeSize === i ? 'active' : ' '} >{size} штук</ li >
                    ))}
                </ul>
            </div>
